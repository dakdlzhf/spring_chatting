...
$(function() {
    ...
    $('#userName').on('keydown', function(key) {
      if (key.keyCode == 13) { // Enter를쳤다면
        // $('#rname').focus();
        chatName(); // 사용자명 입력 Layer를 닫고 메시지 Layer로 변경됨. chatName 메서드 호출
      }  
    });  
})
...
function wsOpen(){
    ws = new WebSocket("ws://" + location.host + "/chat"); // ws://localhost:9092/chat  웹소켓 생성 도메인지정
    wsEvt();
}
